...
void SetNewTaxes(DWORD modBase) {
	...
	int sum = 0;
	for (map<string, int>::iterator it = coeffLocal.begin(); it != coeffLocal.end(); ++it) {
		sum += it->second;
	}
...
int GetFoodConsumptionBonus() {
	map<int, int> foodConsumption = { {0, -8}, {1, -4}, {2, 0}, {3, 4}, {4, 8} };
...
int GetTaxesBonus() {
	map<int, int> taxesBonus = { {0, 7}, {1, 5}, {2, 3}, {3, 1}, {4, -2}, {5, -4}, {6, -6}, {7, -8}, {8, -12}, {9, -16}, {10, -20}, {11, -24} };
...
void FindNewTaxesCoeff(int bonusesSum) {
	...
	Comparator sortAscending =
		[](pair<int, int> tax1, pair<int, int> tax2) {
		return tax1.second < tax2.second;
		};

	set<pair<int, int>, Comparator> setOfTaxes(taxesBonus.begin(), taxesBonus.end(), sortAscending);

	int newTaxesLevel = 3;
	for (pair<int, int> element : setOfTaxes) {
